...
class DQN(OffPolicyAlgorithm):
    ...
    def _on_step(self) -> None:
        ...
        if self.use_second_net and self._n_calls % max(self.target_update_interval // self.n_envs, 1) == 0:
    ...
    def train(self, gradient_steps: int, batch_size: int = 100) -> None:
        ...
        #print(sum(helper_loss)/len(helper_loss))
    ...
    def act(self,
        ...
        if not callback.on_step():
            return RolloutReturn(num_collected_steps * self.env.num_envs, num_collected_episodes, continue_training=False, reward = rewards,done = dones)
    ...
    def train_no_buffer(self, observation, next_observation, reward, done):
        ...
        with th.no_grad():
            # Compute the next Q-values using the target network                   
            next_q_values = self.q_net_target(next_observation) if self.use_second_net else self.q_net(next_observation)
            # Follow greedy policy: use the one with the highest value
            next_q_values, _ = next_q_values.max(dim=1)
            # Avoid potential broadcast issue
            next_q_values = next_q_values.reshape(-1, 1)
            # 1-step TD target
            target_q_values = reward + (1 - done) * discount * next_q_values
        self.policy.set_training_mode(True)   
        # Get current Q-values estimates
